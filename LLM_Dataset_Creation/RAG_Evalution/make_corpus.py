...
@click.option('--dir_path', type=click.Path(exists=True, dir_okay=True, file_okay=False),
              default=os.path.join(root_dir, 'raw_docs')) # 파싱할 디렉토리 경로
...
def main(dir_path: str, save_path: str):
    ...
    documents = SimpleDirectoryReader(dir_path, recursive=True).load_data() #파싱(폴더 경로(dir_path)만 지정하면 확장자와 알맞은 파서와 자동 연결)
    nodes = TokenTextSplitter().get_nodes_from_documents(documents=documents, chunk_size=256, chunk_overlap=64) # 청킹
    corpus_df = llama_text_node_to_parquet(nodes)
    corpus_df = cast_corpus_dataset(corpus_df) #맥북 유저 한글 파일오류 해결위해
